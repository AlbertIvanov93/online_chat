...
public class Server {
    ...
    public static void sendBroadcastMessage(Message message) {
        for (Map.Entry<String, Connection> connectionEntry : connectionMap.entrySet()) {
            try {
                ...
                ConsoleHelper.writeMessage("Не удалось отправить сообщение пользователю: " + connectionEntry.getKey());
            }
        }
    }
    ...
    private static class Handler extends Thread {
        ...
        private void notifyUsers(Connection connection, String userName) throws IOException {
            for (Map.Entry<String, Connection> connectionEntry : connectionMap.entrySet()) {
                String userNameEntry = connectionEntry.getKey();
                if (!userNameEntry.equals(userName)) {
                    Message message = new Message(MessageType.USER_ADDED, userNameEntry);
